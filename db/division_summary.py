...
three thin "wrapper" functions call it with the right grouping column
...
def build_group_comparison(current, prior, group_col, total_label="TOTAL"):
    ...
    (e.g. "division", "department", or "instruction_mode").
    """
...
def build_modality_comparison(current, prior):
    """Modality-level comparison — used on both the landing page (college-wide)
    and the division drill-down page (scoped to one division)."""
