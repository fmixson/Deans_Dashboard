...
def load_data():
    ...
    df = pd.read_sql("""
        SELECT cs.division, cs.department, cs.subject, cs.catalog, cs.descr,
               ...
               cs.start_date, cs.instruction_mode, cs.class_type, ws.class_nbr,
...
def find_exact_en_pair_n(group):
    e_rows = group[group["class_type"] == "E"]
    n_rows = group[group["class_type"] == "N"]
    if len(e_rows) == 1 and len(n_rows) == 1:
        e, n = e_rows.iloc[0], n_rows.iloc[0]
        if e["total_enrolled"] == n["total_enrolled"] and e["enrollment_capacity"] == n["enrollment_capacity"]:
            return n["class_nbr"]
    return None

pair_results = latest.groupby(["subject", "catalog"]).apply(find_exact_en_pair_n, include_groups=False)
exclude_class_nbrs = pair_results.dropna().tolist()
latest.loc[latest["class_nbr"].isin(exclude_class_nbrs), "critically_low"] = False
latest.loc[latest["class_nbr"].isin(exclude_class_nbrs), "low_not_growing"] = False
